      ...
      {loading ? <PageLoading rows={6} /> : error ? <PageError message={error} onRetry={() => void loadTools()} /> : data ? (
        <div className={styles.pageSections}>
          <section className={`${styles.managementSurface} ${styles.operationalSection}`} aria-label="Tool 상태 관리">
            <div className={styles.managementSurfaceHeader}><div><h2 id="tool-status-title" className="type-title">Tool 상태</h2><p className="type-small">등록된 Tool의 공개 상태를 빠르게 확인하고 전환합니다.</p></div></div>
          ...
          <section className={`${styles.managementSurface} ${styles.operationalSection}`} aria-label="Links 데이터 관리">
            <div className={styles.managementSurfaceHeader}><div><h2 id="tool-links-title" className="type-title">Links 데이터</h2><p className="type-small">Links Tool에 노출할 공통 링크 데이터를 관리합니다.</p></div><button className={`${styles.secondaryButton} type-body`} type="button" onClick={() => setEditor({})}><Plus aria-hidden="true" />Link 추가</button></div>
