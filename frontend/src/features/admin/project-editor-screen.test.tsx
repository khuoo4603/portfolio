...
const detail: ProjectDetail = {
  ...
  content: {
    ...
    background: [{ title: null, body: "Background body", mediaId: 9 }],
    features: [{ title: "Feature", description: "Feature description", mediaId: null }],
    development: [{ title: "Backend", items: ["API"], mediaId: null }],
    architecture: { clients: [], services: [], dataAndExternal: [], runtime: [], delivery: [] },
    engineering: [{ title: "Issue", summary: null, problem: "Problem", solution: "Solution", result: "Result", mediaId: null }],
  },
  media: [
    { id: 8, mediaType: "CAROUSEL", imageUrl: "/api/v1/public/projects/media/carousel.webp", label: "Carousel", altText: "Carousel image", displayOrder: 0 },
    { id: 9, mediaType: "CONTENT", imageUrl: "/api/v1/public/projects/media/content.webp", label: "Content", altText: "Content image", displayOrder: 0 },
  ],
};
...
describe("Preview-first Project Editor", () => {
  ...
  it("nullable 기본정보와 고정 6개 Content를 포함한 9개 Section을 안전하게 편집", async () => {
    ...
    expect(screen.getByText("상세 요약 미입력")).toBeInTheDocument();
  ...
  it("기존 비활성 기술은 유지하고 새로운 비활성 기술 선택은 차단", async () => {
    const sectionNav = await renderEditor();
  ...
  it("Content Item에는 CONTENT Media만 연결하고 새 File을 Local Draft에 추가", async () => {
    createObjectUrl.mockReturnValue("blob:content-new");
    const sectionNav = await renderEditor();
    fireEvent.click(within(sectionNav).getByRole("button", { name: /문제 배경/ }));

    const region = screen.getByRole("region", { name: "문제 배경 편집" });
    const select = within(region).getByRole("combobox", { name: "Content Image" });
    expect(within(select).getByRole("option", { name: "Content" })).toBeInTheDocument();
    expect(within(select).queryByRole("option", { name: "Carousel" })).not.toBeInTheDocument();

    fireEvent.change(within(region).getByLabelText("새 이미지"), {
      target: { files: [new File(["content"], "new-content.webp", { type: "image/webp" })] },
    });
    expect(createObjectUrl).toHaveBeenCalledTimes(1);
    expect((select as HTMLSelectElement).value).toMatch(/^client:new-/);
  ...
  it("입력 변경은 Local Draft에만 반영하고 Save 한 번에 PROJECT_UPDATE·재조회 수행", async () => {
    ...
    expect(saveInput.metadata.content.background[0]).toMatchObject({ mediaId: 9, clientKey: null });
